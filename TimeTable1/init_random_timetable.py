#
# This module handles creation of random timetable
#
...
def is_slot_available(req_all, timetable_np, c, r_day, r_slot, req_id, theory_roomgroup, lab_roomgroup, max_theory, max_lab): 
...
def create_random_timetable(n_classes, n_days, n_slots, n_maxlecsperslot, req_all, theory_roomgroup, lab_roomgroup, max_theory, max_lab):
                ...
                for x in batch_sets:
                    ...
                    if(req_batchset<set(x)): #Then we can schedule that group of batches
                        ...
                        for i in req_set:  #Schedule each of these requirements
                            if (is_slot_available(req_all, timetable_np, c, r_day, r_slot, i, theory_roomgroup, lab_roomgroup, max_theory, max_lab)):
    ...
    for c in (set(req_single.classId)):  # First take one class
        ...
        for req in (req_set):  # Schedule each of these requirements
            ...
            while (notassigned == 1 and n_tries<max_tries):  # Keep on scheduling till not found
                ...
                r_slot = random.randint(0,n_slots-1)
                if (is_slot_available(req_all, timetable_np, c, r_day, r_slot, req, theory_roomgroup, lab_roomgroup, max_theory, max_lab)):
